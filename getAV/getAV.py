...
def getav():
        noAV = True
        p = open('tasklist.txt','r')
        contents = p.readlines()
        for content in contents:
            process = content.split(' ')[0] ##以空格作为切割获取进程名称
            # print(process)
            if process in dict:
                noAV = False
                print(dict.get(process))
        if noAV:
            print('无杀软进程')

##字典来源：https://github.com/r00tSe7en/get_AV/blob/master/av.json
dict = {
...
if __name__ == '__main__':
    threading.Thread(target=getav).start()